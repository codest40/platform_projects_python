...
# Pipeline runner
#=============================================
def pipeline_runner(resource, collect, analyze, filter, compute, extra, alert_title=None, message=None, serverity=None):
  result = run_collection(resource=resource, func=collect)
  if result is None:
    print(f"❌ [PIPELINE RUNNER] {resource} ERROR: Emit() response returned: {result}")
      ...
      res = run_analysis(resource=resource, func=analyze, result=result)
      if not res:
